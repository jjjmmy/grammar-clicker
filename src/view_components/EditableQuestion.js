...
export const EditableQuestion = ({ question }) => {
  ...
  const numWords = draftQuestion.text?.split(' ').length || 0;
  const answerKeys = Array.from({ length: numWords }, (_, i) => `answer${String.fromCharCode(65 + i)}`);
    ...
    <div>
      <Card body className="mt-4 mb-4">
        ...
        <FormGroup>
          ...
          >
            {answerKeys.map((ansKey) => (
              <option key={ansKey}>{ansKey.slice(-1)}</option>
            ))}
          </Input>
        </FormGroup>
        {componentState === STATE_SAVING ?
